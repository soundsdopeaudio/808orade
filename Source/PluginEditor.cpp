...
void PluginEditor::paint (juce::Graphics& g)
{
    ...
    g.drawText("808orade", 16, 8, getWidth()-32, 48, juce::Justification::left);
}
...
void PluginEditor::resized()
{
    ...
    // top area for logo handled in paint - waveform area below
...
void PluginEditor::updateWaveformFromProcessor()
{
    ...
    if (currentGeneratedBufferPtr && currentGeneratedBufferPtr->getNumSamples() > 0)
    {
        ...
        seedLabel.setText("Seed: " + juce::String((int64)processor.getLastParams().seed), juce::dontSendNotification);
...
void PluginEditor::regenerateFromCurrentUI()
{
    ...
    gp.sampleRate = getSampleRate() > 0.0 ? getSampleRate() : 44100.0;
    ...
    if (descriptorWindow)
    {
        ...
        // Apply simple mapping rules: for each keyword id, adjust params (examples)
        for (auto& kv : sel)
        {
            ...
            if (id == "subBtn")
                ...
                gp.masterGainDb += 0.5f * intensity; // slight boost then saturation in generator
...
void PluginEditor::buttonClicked(juce::Button* b)
{
    ...
    else if (b == &exportButton)
    {
        ...
        if (chooser.browseForFileToSave(true))
        {
            ...
            bool saved = WavExporter::saveBufferToWav(*bufPtr, processor.getLastParams().sampleRate > 0.0 ? processor.getLastParams().sampleRate : 44100.0, f, 24);
            if (saved)
...
void PluginEditor::showMainMenu()
{
    ...
    {
        // open resynthesis - not implemented here as separate window yet
